...
function loadJSON(callback) {   
  ...
  xobj.open('GET', '/countryheatmap/?display=1', true);
  // 
  xobj.onreadystatechange = function () {
    if (xobj.readyState == 4 && xobj.status == "200") {
      ...
      // Query didn't complete properly
...
function updateScale() {
  ...
  world.objects.countries.geometries.forEach(function(d) {
    if (d.properties.value < min) min = d.properties.value
    if (d.properties.value > max) max = d.properties.value
  })
...
function processWorldTopologyJSON(json) {
  //
...
function draw(topo) {
  ...
  country.enter().insert("path")
      ...
      .style("fill", function(d, i) { return colorScale(d.properties.value); });
  ...
  country
    .on("mousemove", function(d,i) {
      ...
      tooltip.classed("hidden", false)
             ...
             .html(d.properties.name + "<br>" + d.properties.value);
    })
...
loadJSON(processWorldTopologyJSON)
